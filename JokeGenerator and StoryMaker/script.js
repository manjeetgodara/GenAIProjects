document.addEventListener('DOMContentLoaded', function () {
    ...
    generateButton.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/chat?prompt=Don't show introductory part and if multiple jokes are there give jokes in proper format like 1...in one line 2..in another line  and so on. Now write jokes on ${encodeURIComponent(promptText)}`);
